...
{
	fs::path outfile = "common-passwords.txt";
...
//	return 0;
	...
	//  if (!home) {
	//    std::cerr << "Failed to find the home directory." << "'\n";
	//    return EXIT_FAILURE;
	//  }
	//  std::cout << "Chose '" << home << "' as the home directory.\n";
	//  fs::path result_dir = fs::path(home) / "/wordlists";
	//  std::cout << "Chose '" << result_dir << "' as the wordlist directory.\n";
	
	//if (!fs::exists(result_dir)) {
	//    if (!fs::create_directory(result_dir)) {
	//      std::cerr << "Failed to create '" << result_dir << "'\n";
	//      return EXIT_FAILURE;
	//    }
	//  }
	if (fs::exists(outfile))
	{
		std::cout << fs::absolute(outfile)
...
//	}
	...
	std::fprintf(stderr, "# total unique candidates: %zu\n", final.size());
	return 0;
}